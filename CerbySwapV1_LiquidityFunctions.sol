    ...
    CerbySwapV1_ERC1155
{
        ...
        tokenDoesNotExistInPool(token)
    {
        ...
        uint newSqrtKValue = sqrt(amountTokensIn * amountCerUsdToMint);
        ...
        returns (uint)
    {
        ...
        uint newTotalCerUsdBalance = _getTokenBalance(
            cerUsdToken
        );

        uint amountCerUsdIn = newTotalCerUsdBalance
            - totalCerUsdBalance;

        {
            // calculating new sqrt(k) value before updating pool
            uint newSqrtKValue = sqrt(uint(pool.balanceToken)
                * uint(pool.balanceCerUsd));

            // minting trade fees
            uint amountLpTokensToMintAsFee = _getMintFeeLiquidityAmount(
                pool.lastSqrtKValue,
                newSqrtKValue,
                _totalSupply[poolId]
            );
        ...
        { // scope to avoid stack to deep error
            ...
            uint amountCerUsdToMint = amountTokensIn
                * uint(pool.balanceCerUsd)
            ...
            totalCerUsdBalance = totalCerUsdBalance
                + amountCerUsdIn
                + amountCerUsdToMint;

            pool.balanceToken = pool.balanceToken
                + uint128(amountTokensIn);

            pool.balanceCerUsd = pool.balanceCerUsd
                + uint128(amountCerUsdIn
                + amountCerUsdToMint);

            pool.lastSqrtKValue = uint128(sqrt(uint(pool.balanceToken)
                * uint(pool.balanceCerUsd)));
