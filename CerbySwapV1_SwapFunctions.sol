    ...
    CerbySwapV1_GetFunctions
{
        ...
        returns (uint, uint)
    {
        ...
        // TODO: uncomment below
            ...
            // getting amountTokensOut=
            uint amountCerUsdOut = _getOutputExactTokensForCerUsd(
                tokenIn,
                amountTokensIn
            );
        ...
        tokenMustExistInPool(token)
    {
        ...
        uint oneMinusFee = amountCerUsdIn > 1 && amountTokensIn <= 1
            ? FEE_DENORM
        ...
        { // scope to avoid stack too deep error
            ...
            uint beforeKValueDenormed = uint(pool.balanceCerUsd)
                * FEE_DENORM
                * uint(pool.balanceToken)
                * FEE_DENORM;

            // calculating new pool values
            uint _totalCerUsdBalance = totalCerUsdBalance
                + amountCerUsdIn
                - amountCerUsdOut;

            uint _balanceCerUsd = uint(pool.balanceCerUsd)
                + amountCerUsdIn
                - amountCerUsdOut;

            uint _balanceToken = uint(pool.balanceToken)
                + amountTokensIn
                - amountTokensOut;

            // calculating new K value including trade fees
            uint afterKValueDenormed = (_balanceCerUsd
                * FEE_DENORM
                - amountCerUsdIn
            ...
            if (pool.creditCerUsd < type(uint).max) {
                pool.creditCerUsd = pool.creditCerUsd
                    + amountCerUsdIn
                    - amountCerUsdOut;
            }
        }
        ...
        uint nextPeriod = (currentPeriod + 1)
        ...
        if (amountCerUsdOut > TRADE_VOLUME_DENORM) { // or else amountCerUsdOut / TRADE_VOLUME_DENORM == 0
            ...
            uint updatedTradeVolume = uint(pool.tradeVolumePerPeriodInCerUsd[currentPeriod])
                + amountCerUsdOut
                / TRADE_VOLUME_DENORM; // if ANY --> cerUSD, then output is cerUSD only
