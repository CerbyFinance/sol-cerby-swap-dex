...
contract('Cerby', (accounts) => {
    ...
    it('addTokenLiquidity: add 1041e10 ETH; pool must be updated correctly', async () => {
        ...
        const beforeEthPool = (await cerbySwap.getPoolsByTokens([ETH_TOKEN_ADDRESS]))[0];
        const beforeLpTokens = await cerbySwap.balanceOf(firstAccount, ETH_POOL_POS);
        {
            ...
            const afterEthPool = (await cerbySwap.getPoolsByTokens([ETH_TOKEN_ADDRESS]))[0];
    ...
    it('hacks: buy 1000e12 CERBY, add liquidity, 3 trades CERBY --> cerUSD --> CERBY, remove liquidity; result CERBY <= 1000e12', async () => {
        ...
        const CERBY_POOL_POS = await cerbySwap.getTokenToPoolId(TestCerbyToken.address);
        {
            ...
            // buying 2000 CERBY
            await cerbySwap.swapTokensForExactTokens(ETH_TOKEN_ADDRESS, TestCerbyToken.address, amountTokensOut, maxAmountTokensIn, expireTimestamp, transferTo, { value: maxAmountTokensIn });
    ...
    // ---------------------------------------------------------- //
    it.skip('getCurrentOneMinusFeeBasedOnTrades: time travel, do small trade, time travel, check fee', async () => {
        await delay(DELAY_BETWEEN_TESTS);
        const accounts = await web3.eth.getAccounts();
        const firstAccount = accounts[0];
        const cerbySwap = await CerbySwapV1.deployed();
        {
            const snapshotId = await send({
                method: 'evm_snapshot',
                params: [],
            });
            const ONE_PERIOD = 17280; // 4.8 hours time travel
            const startPeriod = await cerbySwap.getCurrentPeriod();
            await increaseTime(ONE_PERIOD);
            const endPeriod = await cerbySwap.getCurrentPeriod();
            assert.deepEqual(endPeriod.toString(), startPeriod.add(_BN(1)).toString()); // period shifted +1
            await increaseTime(ONE_PERIOD * 10); // shifting 2 days to clear any fees stats
            let actualOneMinusFee = await cerbySwap.getCurrentOneMinusFeeBasedOnTrades(TestCerbyToken.address);
            ...
            const beforeCerbyPool = (await cerbySwap.getPoolsByTokens([TestCerbyToken.address]))[0];
            const cerbyToken = await TestCerbyToken.deployed();
            await cerbyToken.mintHumanAddress(firstAccount, _BN(beforeCerbyPool.balanceToken).mul(_BN(31)));
            const tokenIn = TestCerbyToken.address;
            const tokenOut = TestCerUsdToken.address;
    ...
    it('reduce creditCerUsd, try swapping CERBY --> cerUSD: must revert', async () => {
        ...
        const cerbySwap = await CerbySwapV1.deployed();
        {
            ...
            const transferTo = firstAccount;
            // buying CERBY
            await cerbySwap.swapTokensForExactTokens(ETH_TOKEN_ADDRESS, TestCerbyToken.address, amountTokensIn, bn1e18, expireTimestamp, transferTo, { value: bn1e18, gas: '30000000' });
